		...
		<tr>
			<td>
				<img
					...
					class="w-52 h-52 object-center"
				/>
			</td>
			<td>
				<h1 class="font-bold text-2xl">User: {user.username}</h1>
				{#if user.about_me}
					<p class="my-5">{user.about_me}</p>
				{/if}
				{#if user.last_seen}
					<p class="text-gray-500 italic">Last seen on: {user.last_seen}</p>
				{/if}
				{#if user.id === current_user?.id}
					<a
						href="/users/{current_user?.username}/edit"
						class="underline text-blue-700 block mt-3 text-sm"
						>Edit your profile</a
					>
				{/if}
			</td>
